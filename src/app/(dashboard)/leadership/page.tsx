...
export default function LeadershipDashboardPage() {
  ...
  const kpis = [
    { title: "Total Projects", value: "0", icon: Building2, color: "text-blue-600", bg: "bg-blue-100" },
    { title: "Active Students", value: "0", icon: Users, color: "text-indigo-600", bg: "bg-indigo-100" },
    { title: "Completed Projects", value: "0", icon: CheckCircle2, color: "text-emerald-600", bg: "bg-emerald-100" },
    { title: "Pending Review", value: "0", icon: Clock, color: "text-amber-600", bg: "bg-amber-100" },
    { title: "Avg. SDG Impact", value: "0/10", icon: Target, color: "text-purple-600", bg: "bg-purple-100" },
    { title: "Faculty Engagement", value: "0%", icon: Activity, color: "text-rose-600", bg: "bg-rose-100" },
  ];
    ...
    <div className="flex flex-col gap-6 p-6 xl:p-8 max-w-[1600px] mx-auto w-full animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1 max-w-2xl">
            Real-time SDG analytics and operational overview strictly scoped to <span className="font-semibold text-slate-700 bg-slate-100 px-2 py-0.5 rounded-md">{userScope}</span>.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
        {kpis.map((kpi, i) => {
          const Icon = kpi.icon;
          return (
            <Card key={i} className="rounded-xl shadow-sm border border-slate-200 hover:shadow-md transition-shadow">
              <CardContent className="p-4 flex flex-col justify-between h-full gap-3">
                ...
                <div>
                  ...
                  <div className="text-[13px] font-medium text-slate-500 mt-0.5">{kpi.title}</div>
                </div>
              </CardContent>
            </Card>
          );
                ...
                <tbody className="divide-y divide-slate-100">
                  {departmentFaculty.map((faculty, i) => (
                    <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-slate-600 font-mono text-xs">{faculty.id}</td>
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium ${
                          faculty.status === "Active" ? "bg-emerald-100 text-emerald-700" :
                          faculty.status === "On Leave" ? "bg-rose-100 text-rose-700" :
