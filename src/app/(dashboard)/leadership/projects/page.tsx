...
export default function LeadershipProjectsPage() {
  ...
  // Zero-Mock Data Policy: Empty until connected to backend
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-gray-100 flex items-center gap-3">
            <Folder className="text-blue-600 dark:text-blue-500" size={32} />
            Projects Oversight
          </h1>
          <p className="text-slate-500 dark:text-gray-400 mt-1 max-w-2xl">
            Monitor and filter student projects aligned with Sustainable Development Goals across <span className="font-semibold text-slate-700 dark:text-gray-200 bg-slate-100 dark:bg-gray-800 px-2 py-0.5 rounded-md">{userScope}</span>.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="rounded-xl shadow-sm border border-slate-200 dark:border-gray-800 bg-white dark:bg-[#1F2937]">
          <CardContent className="p-4 flex items-center gap-4">
            <div className="p-3 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-lg"><Folder size={24} /></div>
            <div>
              <p className="text-sm font-medium text-slate-500 dark:text-gray-400">Total Projects</p>
              <h3 className="text-2xl font-bold text-slate-900 dark:text-gray-100">{totalProjects}</h3>
            </div>
          </CardContent>
        </Card>
        <Card className="rounded-xl shadow-sm border border-slate-200 dark:border-gray-800 bg-white dark:bg-[#1F2937]">
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-medium text-slate-500 dark:text-gray-400">Successfully Completed</p>
              <h3 className="text-2xl font-bold text-slate-900 dark:text-gray-100">{completedProjects}</h3>
            </div>
          </CardContent>
        </Card>
        <Card className="rounded-xl shadow-sm border border-slate-200 dark:border-gray-800 bg-white dark:bg-[#1F2937]">
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-medium text-slate-500 dark:text-gray-400">Pending Review</p>
              <h3 className="text-2xl font-bold text-slate-900 dark:text-gray-100">{underReviewProjects}</h3>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="rounded-xl shadow-sm border border-slate-200 dark:border-gray-800 bg-white dark:bg-[#1F2937]">
        <CardHeader className="bg-slate-50/50 dark:bg-gray-800/50 border-b border-slate-100 dark:border-gray-800 pb-4">
          <div className="flex flex-col xl:flex-row items-center gap-4 justify-between">
            <CardTitle className="text-lg font-bold text-slate-800 dark:text-gray-100">Project Directory</CardTitle>
            <div className="flex flex-col sm:flex-row flex-wrap items-center gap-3 w-full xl:w-auto">
              ...
              <select
                className="flex h-10 w-full sm:w-36 rounded-md border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 text-gray-900 dark:text-gray-100"
                value={selectedYear}
                ...
                {filteredProjects.length > 0 ? (
                  filteredProjects.map((project, i) => (
                    <tr key={i} className="hover:bg-slate-50/80 dark:hover:bg-gray-800/30 transition-colors bg-white dark:bg-[#1F2937]">
                      <td className="px-6 py-4">
                        <div className="font-semibold text-slate-900 dark:text-gray-100">{project.title}</div>
                        <div className="text-xs text-slate-500 dark:text-gray-400 mt-0.5">
                          {project.studentName} <span className="font-mono ml-1 text-slate-400 dark:text-gray-500">({project.studentId})</span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-center">
                        <span className="inline-flex items-center justify-center bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 text-xs font-bold px-2.5 py-0.5 rounded-full whitespace-nowrap">
                          {project.sdg}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-slate-600 dark:text-gray-300 flex items-center gap-2">
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-slate-500 dark:text-gray-400">
                      ...
                      No projects found matching your criteria. Awaiting new student submissions.
                    </td>
                  </tr>
                )}
              </tbody>
